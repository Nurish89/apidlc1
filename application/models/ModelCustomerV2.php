...
class ModelCustomerV2 extends CI_Model
{
	...
	public function getCustomer($chekType, $checkValue)
	{
        ...
        public function getCustomerLike($checkType, $checkValue){
		$checkStr = " and $checkType like %?%";
                $dataArray = array($checkValue);
                $sql = "select id from customerv2 where isDeleted = 0  and $checkType like '%$checkValue%'";
	...
	public function updateCustomer($data, $whereData)
        {
                foreach($whereData as $k => $v)
	                $this->db->where($k,$v);
                $this->db->update('customerv2',$data);
                return $this->db->affected_rows();
        }	
